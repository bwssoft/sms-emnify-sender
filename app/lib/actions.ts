...
export async function userAuthenticate(formData: FormData) {
	...
	const result = await emnify.authenticate({
		token: process.env.BWS_EMNIFY_AUTH_TOKEN,
	});
	if (!result) return;
...
export async function fetchEndpointsFilteredByName(
	value: string,
	type?: string,
) {
	// return await emnify.listEndpointsFilteredByName({ name })
	return await repository.listFilteredSimcardByEndpointName({
		value,
		type: type ?? 'endpoint_name',
	});
	// await new Promise<void>((resolve) => setTimeout(resolve, 2000))
	// return endpoints.filter(el => el.name.toLocaleLowerCase().includes(name.toLocaleLowerCase()))
}

export async function sendMessagefromMessagePage(
	params: { endpoint_id: string; url: string },
	formData: FormData,
) {
...
export async function getMessagefromEndpoint(
	device_id: string,
	sms_id: string,
) {
...
export async function refreshMessageDatafromEndpoint({
	device_id,
	sms_id,
}: {
	device_id: string;
...
export async function refreshMessageDatafromEndpointMessagePage({
	device_id,
	sms_id,
}: {
	device_id: string;
